...
未设置 *PID* 时在全局范围内捕获 seccomp 过滤器。eBPF 程序将追踪 `seccomp_check_filter` 内核函数，可靠地拦截内核将加载的 seccomp 过滤器，而相比 <<trace - 运行时捕获过滤器>> 没有条件竞争的风险。这个功能在下方被称为“全局捕获”。Linux 5.15 是运行该功能的最低推荐内核版本。
