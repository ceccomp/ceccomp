...
-f, --format *FMT*::
决定了 `ceccomp` 如何输出二进制格式的 BPF 字节码。可以是 _hexfmt_、_hexline_、_raw_ 或者 _c-macro_。你可以在 <<ceccomp 示例>> 节中找到示例输出。默认值是 _hexline_。
...
NOTE: 从 _4.3 版本_ 开始，新的 *FMT* _c-macro_ 被加入用以轻松将 seccomp 规则移植到其他架构上。
...
-e, --ebpf::
认为 _RAW_ 是 eBPF 指令字节而非 cBPF 过滤器字节。这个标志被设计用于测试 eBPF 到 cBPF 的转换是否正常工作。要使用这个功能，请使用 eBPF 或内核模块导出进程内安装的从 seccomp 过滤器转换过来的 eBPF 指令。任何不是由内核生成的输入都可能产生未定义行为。
...
-p, --pid *PID*::
设置为你想检查的 pid。 *PID* 和 *PROGRAM* 相冲突；你只能在一条命令中动态运行一个程序，或者检查一个 pid。没有 `-s` 标志，trace pid 会尝试使用 `ptrace(PTRACE_SECCOMP_GET_FILTER)` 从 *PID* 中提取 seccomp 过滤器，这个操作在一些系统上可能不可用。这里的 pid 其实是进程（线程）ID，你可以追踪一个线程或进程。
...
=== capture - 用 eBPF 捕获过滤器

    ceccomp capture [-c WHEN] [-p PID]

未设置 *PID* 时在全局范围内捕获 seccomp 过滤器。eBPF 程序将追踪 `seccomp_check_filter` 内核函数，可靠地拦截内核将加载的 seccomp 过滤器，而相比 <<trace - 运行时捕获过滤器>> 没有条件竞争的风险。这个功能在下方被称为“全局捕获”。在 _x86_64_ 上，Linux 5.15 是运行该功能的最低推荐内核版本。

当设置了 *PID* 时，将安装用来从对应进程（其实是线程）中提取已安装过的 seccomp 过滤器的 eBPF 程序。注意在内核中，`pid` 指线程 id，而 `tgid` 指进程 id。eBPF 程序会直接把由原始 seccomp 过滤器转换后的 eBPF 指令抽取出来并转换回去，用以执行 <<disasm - 反汇编>>。这个功能不需要 `CONFIG_CHECKPOINT_RESTORE` 内核配置，甚至在 `ceccomp` 处在 seccomp 状态时仍能工作。这个功能在下方被称为“捕获 PID”。Linux 6.2 是运行该功能的最低内核版本，因为这时 `bpf_task_from_pid` 函数才被引入。

IMPORTANT: 当内核迁移 seccomp 过滤器时，由于一些不同的 cBPF 指令会被映射到相同的 eBPF 指令序列，因此从 cBPF 到 eBPF 的转换是有损的。结果是“捕获 PID”的输出可能和原始的 seccomp 过滤器并不一致，尽管它们在功能上是等价的。

目前，compat task 检测只支持 _x86_64_ 和 _aarch64_。 这个功能允许 `ceccomp` 识别一个任务是原生的 (64 位内核上的 64 位任务)，还是“兼容的” (64 位内核上的 32 位任务)，由此 `ceccomp` 才能把正确的架构标识传给 libseccomp 来查询捕获到的 seccomp 过滤器中的系统调用的名字。

注意 eBPF 利用异步环形缓冲区来向用户态进程发回数据，所以如果同时加载了好多 seccomp 过滤器，缓冲区可能会耗尽并且您需要运行 `bpftool prog trace` 来看到详细错误。由于我们不能将错误通过满的缓冲区发回来，因此那些溢出的过滤器对没有阅读内核消息的用户来说实际上是不可见的。

-c, --color *WHEN*::
参数描述可以在 <<asm - 汇编>> 一节中找到。

-p, --pid *PID*::
从哪个 *进程* (线程) 中提取 seccomp 过滤器指令。

TIP: 所有出现在“全局捕获”或“捕获 PID”中的 *PID* 是 *初始命名空间 PID*，而不是当前命名空间中的 PID。推荐只在初始命名空间中运行 `ceccomp capture`。

NOTE: 此模块是可选的，您可以在配置阶段将其禁用。此模块需要 libbpf 才能工作。

NOTE: 此子命令在 _4.3 版本_ 中被引入。

== TEXT 语法参考

IMPORTANT: _4.0 版本_ 提高了词法分析器代码的可读性，伴随着大幅修改的语法。前缀为 `#` 的行现在是注释了。同时 _行号_ 被替换为 _标签_，现在词法分析器根据标签定义来决定跳转到哪里，而不是根据 _TEXT_ 文件中的行号。

这里有正确的 _TEXT_ 格式，以类 EBNF 语法描述：https://github.com/ceccomp/ceccomp/issues/17#issuecomment-3610531705 。 没兴趣了解 EBNF？请继续阅读以下的例子。
...
CAUTION: `ceccomp disasm` 和 `seccomp-tools disasm` 的输出之间有很多细微的差别， 以下是一个典型的输出示例。同时有些伪代码是不同的，所以不要盲目将 seccomp-tools 的输出管道给 ceccomp。
...
在 https://github.com/ceccomp/ceccomp 可以找到源代码。 欢迎提交 Pull Requests 和 Issues ！
