...
=== disasm - DISASSEMBLE
    ...
    ceccomp disasm [-c WHEN] [-a ARCH] [-e] [RAW]
...
-e, --ebpf::
Treat _RAW_ as eBPF insn bytes instead of cBPF filter bytes. This flag is designed to
test whether eBPF-to-cBPF conversion is working or not. To use this function, please
dump installed eBPF insns migrated from seccomp filters in a thread via eBPF or a
kernel module.
...
-q, --quiet::
Set to suppress the extra *[INFO]* prints when detect thread forking, exiting or
...
NOTE: Since _version 3.1_, multiple thread tracing is introduced, and when tracee
...
=== capture - CAPTURE FILTERS WITH EBPF

    ceccomp capture [-c WHEN] [-p PID]

Capture seccomp filters in global scope if *PID* not set. The eBPF program
will trace `seccomp_check_filter` kernel function, reliably intercepting seccomp
filters that the kernel will load, WITHOUT race conditions compared to
<<trace - TRACE FILTER IN RUNTIME>>. This function is named "global capture"
below. Linux 5.15 is the lowest recommended kernel version to run this function
on _x86_64_, other architecture may require higher kernel version to run this
function.

If *PID* is set, an eBPF program to extract installed seccomp filters in the
corresponding thread will be installed. Note that inside kernel, `pid` is
referred thread id and `tgid` is referred to process id. The eBPF program
directly pulls eBPF insns migrated from original seccomp filters out and
converts them back to perform <<disasm - DISASSEMBLE>>. This function does
not require `CONFIG_CHECKPOINT_RESTORE` kernel config, and is functional
even if `ceccomp` itself is under seccomp. This function is named "capture pid"
below. Linux 6.2 is the lowest kernel version to run this function, as
`bpf_task_from_pid` was introduced then.

Currently, compat task detection is only supported on _x86_64_ and _aarch64_.
With this feature, `ceccomp` can identify whether a task is native (64-bit
task on 64-bit kernel) or "compatible" (32-bit task on 64-bit kernel), so that
`ceccomp` can pass the correct architecture token to libseccomp to query
syscall name in captured seccomp filters.

WHEN::
Argument description can be found in <<asm - ASSEMBLE>> section.

PID::
Which *thread* to extract seccomp filter insns from.

NOTE: All *PID* presented in either "global capture" or "capture pid" are
*INITIAL NAMESPACE PID*, not PID in current namespace. It's recommended that
only run `ceccomp capture` in initial namespace.

NOTE: This module is optional, you can opt it out during configuration. This
module require libbpf to work.

NOTE: This subcommand was introduced in _version 4.3_.
...
https://github.com/ceccomp/ceccomp/issues/17#issuecomment-3610531705 .
...
Capture filters in a thread:

image::capture_pid.webp[]
endif::[]

== REPO

Visit https://github.com/ceccomp/ceccomp to find the code.
