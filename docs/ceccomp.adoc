...
dump installed eBPF insns migrated from seccomp filters in a process via eBPF or a
kernel module.
...
corresponding process will be installed. Note that inside kernel, `pid` is
...
PID::
Which *process* to extract seccomp filter insns from.
